...
public class LetterDecorator extends RecyclerView.ItemDecoration {
    ...
    public LetterDecorator(Context context, List<String> data) {
        mContext = context;
        mData = data;
    ...
    private void drawLetterToItemLeft(Canvas c, RecyclerView parent) {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //当前名字拼音的第一个字母
            String letter = mData.get(position).charAt(0) + "";
            if (position == 0) {
                drawLetter(letter, width, height, c, parent);
            } else {
                String preLetter = mData.get(position - 1).charAt(0) + "";
